    ...
    runbooks = [
        {
            ...
            "title": "MongoDB Collection Scan (COLLSCAN) & CPU Saturation Remediation",
            "tier": "database_storage",
            "root_cause_pattern": "Unindexed `db.cart_items.find({user_id: ..., status: 'PENDING'})` scans all 10,000 documents, causing high latency.",
            "remediation_protocol": "1. Run explain() to verify stage is COLLSCAN. 2. Apply compound index: `db.cart_items.createIndex({ user_id: 1, status: 1 }, { name: 'idx_cart_items_user_status' })`. 3. Verify stage switches to IXSCAN with sub-5ms query response."
        }
    ]
    return f"Retrieved Engineering Runbook from Knowledge Base:\n{runbooks[0]}"
...
def analyze_database_locks(database_cluster: str) -> str:
    ...
    return (
        ...
        f"• Root Cause: Missing compound index on (user_id, status) causing MongoDB to scan all {diag['total_documents']} documents."
    )
...
def execute_sandbox_tests(patch_code: str, test_code: str) -> str:
    ...
    try:
        ...
        if result.returncode == 0:
            return (
                f"✅ REAL LIVE MONGODB VERIFICATION: ALL TESTS PASSED\n"
                f"• Target Server: MongoDB v7.0.12 @ mongodb://localhost:27017/\n"
                f"• Live Index Created: `{idx_name}` on collection `{new_diag['collection']}`\n"
                f"• Real MongoDB Query Stage: {new_diag['stage']}\n"
                f"• Full Collection Scan (COLLSCAN): {new_diag['is_collscan']} (Optimized to IXSCAN)\n"
                f"• New Query Execution Latency: {new_diag['execution_duration_ms']:.2f}ms (Sub-Millisecond Response)\n"
